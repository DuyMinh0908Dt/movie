...
def search_movies(content: str):
    ...
    if not result.empty:
        return {
            "matches": result[['MaPhim', 'TenPhim', 'HinhAnh', 'Trailer']].to_dict(orient='records')
        }
...
def search_image_content(query_image: bytes):
    ...
    for idx in ids:
        ...
        movies_df["HinhAnh"] = movies_df["HinhAnh"].apply(lambda x: x.split("/")[-1])
        matched_row = movies_df[movies_df["HinhAnh"] == img_name]
        if not matched_row.empty:
            ...
            results.append({
                ...
                "HinhAnh": "public/uploads/" + row["HinhAnh"],
                "Trailer": row["Trailer"]
            })
    return results
import asyncio
